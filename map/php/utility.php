...
class URLHelper {
  ...
  /* 
    ...
    Set escapeSQL to false if you do not wish to escape special sql characters
  */
  public function getParameter($paramName, $escapeSQL = true){
    $toReturn = null;
    //check both post and get params
    if(isset($this->parameters_[$paramName])) {
      $toReturn = $this->parameters_[$paramName];
      
      if($escapeSQL) {
        $toReturn = $this->dbHelper_->escapeString($toReturn);
      }
